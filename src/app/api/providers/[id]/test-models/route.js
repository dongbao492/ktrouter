 ...
 */
async function pingModel(modelId, baseUrl, apiKey, cookie) {
...
async function pingImageModel(modelId, baseUrl, apiKey, cookie) {
  const start = Date.now();
  try {
    const headers = { "Content-Type": "application/json" };
    if (apiKey) headers["Authorization"] = `Bearer ${apiKey}`;
    ...
    const res = await fetch(`${baseUrl}/v1/images/generations`, {
      ...
      body: JSON.stringify({
        model: modelId,
        prompt: "A small red square icon on a plain white background",
        n: 1,
    ...
    if (!res.ok) {
      ...
      return { ok: false, latencyMs, error: `HTTP ${res.status}${detail ? `: ${String(detail).slice(0, 120)}` : ""}` };
    }
    const firstImage = parsed?.data?.[0];
    if (!firstImage?.b64_json && !firstImage?.url) {
      return { ok: false, latencyMs, error: "Provider returned no image data" };
    }
...
async function pingVideoModel(modelId, baseUrl, apiKey, cookie) {
  const start = Date.now();
  try {
    const headers = { "Content-Type": "application/json" };
    if (apiKey) headers["Authorization"] = `Bearer ${apiKey}`;
    ...
    const res = await fetch(`${baseUrl}/v1/video/generations`, {
      ...
      body: JSON.stringify({
        model: modelId,
        prompt: "A beautiful scenery of mountains at sunset",
      }),
      signal: AbortSignal.timeout(60000),
    });
    const latencyMs = Date.now() - start;
    ...
    if (!firstVideo?.url) {
      return { ok: false, latencyMs, error: "Provider returned no video data" };
    }
    return { ok: true, latencyMs, error: null };
  } catch (err) {
    return { ok: false, latencyMs: Date.now() - start, error: err.message };
...
function pingProviderModel(model, fullModelId, baseUrl, apiKey, cookie) {
  if (model.type === "image") {
    return pingImageModel(fullModelId, baseUrl, apiKey, cookie);
  }
  if (model.type === "video") {
    return pingVideoModel(fullModelId, baseUrl, apiKey, cookie);
  }
  return pingModel(fullModelId, baseUrl, apiKey, cookie);
}
...
export async function POST(request, { params }) {
  try {
    ...
    const baseUrl = `http://localhost:${process.env.PORT || UPDATER_CONFIG.appPort}`;
    ...
    const firstResult = await pingProviderModel(first, `${alias}/${first.id}`, baseUrl, apiKey, cookie);
    const results = [{ modelId: first.id, name: first.name || first.id, ...firstResult }];

    if (rest.length > 0) {
      const restResults = await Promise.all(
        rest.map(async (model) => {
          const result = await pingProviderModel(model, `${alias}/${model.id}`, baseUrl, apiKey, cookie);
