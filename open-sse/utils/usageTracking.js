...
import { saveRequestUsage, appendRequestLog, calculateCost } from "@/lib/usageDb.js";
 ...
 */
export async function logUsage(provider, usage, model = null, connectionId = null, apiKey = null) {
  ...
  try {
    const cost = await calculateCost(provider, model, { prompt_tokens: inTokens, completion_tokens: outTokens, cache_read_input_tokens: usage.cache_read_input_tokens || usage.cached_tokens || 0, reasoning_tokens: usage.reasoning_tokens || 0 });
    if (cost > 0) msg += ` | ${COLORS.cyan}cost=${cost.toFixed(4)}${COLORS.reset}`;
  } catch {}
