...
const parseCodexConfig = (config) => {
  ...
  const effortMatch = config.match(/^model_reasoning_effort\s*=\s*"([^"]+)"/im);
  const wireMatch = config.match(/wire_api\s*=\s*"([^"]+)"/im);
  const subagentModelMatch = config.match(/\[agents\.subagent\]\s*\n\s*model\s*=\s*"([^"]+)"/m);

  return {
    model: modelMatch?.[1] || "",
    reasoningEffort: effortMatch?.[1] || "xhigh",
    wireApi: wireMatch?.[1] || "responses",
...
export default function CodexToolCard({ tool, isExpanded, onToggle, baseUrl, apiKeys, activeProviders, cloudEnabled, initialStatus }) {
  const initialConfig = parseCodexConfig(initialStatus?.config);
  ...
  const [subagentModel, setSubagentModel] = useState(initialConfig.subagentModel);
  const [reasoningEffort, setReasoningEffort] = useState(initialConfig.reasoningEffort);
  const [wireApi, setWireApi] = useState(initialConfig.wireApi);
  const [modalOpen, setModalOpen] = useState(false);
  const [subagentModalOpen, setSubagentModalOpen] = useState(false);
  const [modelAliases, setModelAliases] = useState({});
  const [showManualConfigModal, setShowManualConfigModal] = useState(false);
  const [customBaseUrl, setCustomBaseUrl] = useState("");
  const currentApiKey = selectedApiKey || apiKeys?.[0]?.key || "";
  const selectedModelIsDirect = isDirectProviderModel(selectedModel);
  const subagentModelIsDirect = !subagentModel || isDirectProviderModel(subagentModel);
  const canApplySettings = !!selectedModel
    && selectedModelIsDirect
    && subagentModelIsDirect
    && !(!currentApiKey && (cloudEnabled && apiKeys.length > 0));

  const applyConfigToForm = useCallback((config) => {
    const parsed = parseCodexConfig(config);
    setSelectedModel(parsed.model);
    setSubagentModel(parsed.subagentModel);
    setReasoningEffort(parsed.reasoningEffort);
    setWireApi(parsed.wireApi);
  }, []);
  ...
  useEffect(() => {
    ...
    if (!codexStatus) {
      Promise.resolve().then(checkCodexStatus);
    }
    Promise.resolve().then(fetchModelAliases);
  ...
  const handleApplySettings = async () => {
    setMessage(null);
    if (!selectedModelIsDirect) {
      setMessage({ type: "error", text: "Model must be a direct provider/model value. Combos and bare aliases are not written to Codex config." });
      return;
    }
    if (!subagentModelIsDirect) {
      setMessage({ type: "error", text: "Subagent model must also use provider/model format, or leave it empty to use the main model." });
      return;
    }

    setApplying(true);
    try {
      // Use sk_quoctai for localhost if no key, otherwise use selected key
      const keyToUse = (currentApiKey && currentApiKey.trim())
        ? currentApiKey
        : (!cloudEnabled ? "sk_quoctai" : currentApiKey);
  ...
  const getManualConfigs = () => {
    const keyToUse = (currentApiKey && currentApiKey.trim())
      ? currentApiKey
      : (!cloudEnabled ? "sk_quoctai" : "<API_KEY_FROM_DASHBOARD>");

    const effectiveSubagentModel = subagentModel || selectedModel;
    const effortLine = reasoningEffort && reasoningEffort !== "none"
      ? `model_reasoning_effort = "${reasoningEffort}"\n`
      : "";

    const configContent = `# QuocTai Configuration for Codex CLI
model = "${selectedModel}"
model_provider = "quoctai"
${effortLine}
[model_providers.quoctai]
name = "QuocTai"
            ...
            <>
              <div className="flex flex-col gap-2">
                ...
                <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-[8rem_auto_1fr_auto] sm:items-center sm:gap-2">
                  ...
                  <span className="material-symbols-outlined hidden text-text-muted text-[14px] sm:inline">arrow_forward</span>
                  <ApiKeySelect value={currentApiKey} onChange={setSelectedApiKey} apiKeys={apiKeys} cloudEnabled={cloudEnabled} />
                </div>
                ...
                {selectedModel && !selectedModelIsDirect && (
                  <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-[8rem_auto_1fr] sm:items-center sm:gap-2">
                    <span className="hidden sm:block" />
                    <span className="hidden sm:block" />
                    <p className="text-xs text-red-500">Use a direct provider/model value. Codex config no longer accepts combos or bare aliases.</p>
                  </div>
                )}
                ...
                {subagentModel && !subagentModelIsDirect && (
                  <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-[8rem_auto_1fr] sm:items-center sm:gap-2">
                    <span className="hidden sm:block" />
                    <span className="hidden sm:block" />
                    <p className="text-xs text-red-500">Use provider/model here too, or leave blank to mirror the main model.</p>
                  </div>
                )}

                <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-[8rem_auto_1fr] sm:items-center sm:gap-2">
                  <span className="text-xs font-semibold text-text-main sm:text-right sm:text-sm">Reasoning Effort</span>
                  <span className="material-symbols-outlined hidden text-text-muted text-[14px] sm:inline">arrow_forward</span>
                  <select value={reasoningEffort} onChange={(e) => setReasoningEffort(e.target.value)} className="w-full px-2 py-2 bg-surface rounded border border-border text-xs focus:outline-none focus:ring-1 focus:ring-primary/50 sm:py-1.5">
                ...
                <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-[8rem_auto_1fr] sm:items-center sm:gap-2">
                  <span className="text-xs font-semibold text-text-main sm:text-right sm:text-sm">Wire API</span>
                  <span className="material-symbols-outlined hidden text-text-muted text-[14px] sm:inline">arrow_forward</span>
                  <select value={wireApi} onChange={(e) => setWireApi(e.target.value)} className="w-full px-2 py-2 bg-surface rounded border border-border text-xs focus:outline-none focus:ring-1 focus:ring-primary/50 sm:py-1.5">
              ...
              <div className="grid grid-cols-1 gap-2 sm:flex sm:items-center">
                <Button variant="primary" size="sm" onClick={handleApplySettings} disabled={!canApplySettings} loading={applying}>
