...
export default function ModelSelectModal({
  ...
  useEffect(() => {
    if (!isOpen) return;
    Promise.resolve().then(fetchCombos);
  }, [isOpen]);
  ...
  useEffect(() => {
    if (!isOpen) return;
    Promise.resolve().then(fetchProviderNodes);
  }, [isOpen]);
  ...
  useEffect(() => {
    if (!isOpen) return;
    Promise.resolve().then(fetchCustomModels);
  }, [isOpen]);
  ...
  useEffect(() => {
    if (!isOpen) return;
    Promise.resolve().then(fetchDisabledModels);
  }, [isOpen]);
