...
function buildMenuItems(port, autostartEnabled) {
  return [
    { title: `KTRouter (Port ${port})`, tooltip: "Server is running", enabled: false },
  ...
  try {
    ...
    if (isV2) {
      ...
      trayInstance.ready().catch((err) => {
        process.stderr.write(`[ktrouter] tray failed to start: ${err && err.message ? err.message : err}\n`);
      });
    } else {
    ...
    process.stderr.write(`[ktrouter] tray init error: ${err.message}\n`);
    return null;
  }
}
