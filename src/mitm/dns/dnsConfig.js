...
function parseHostsEntries(content) {
  const entries = [];
  for (const rawLine of String(content || "").split(/\r?\n/)) {
    ...
    if (!address || parts.length === 0) continue;

    for (const host of parts) {
      entries.push({ address: address.toLowerCase(), host: host.toLowerCase() });
    }
  }
  return entries;
}

function isLocalMitmAddress(address) {
  return address === "127.0.0.1" || address === "::1";
}

function hasLocalHostEntry(entries, host) {
  const needle = String(host || "").toLowerCase();
  return entries.some((entry) => entry.host === needle && isLocalMitmAddress(entry.address));
}
...
function checkDNSEntry(host = null) {
  try {
    ...
    const entries = parseHostsEntries(hostsContent);
    if (host) return hasLocalHostEntry(entries, host);
    // Legacy: check all antigravity hosts (backward compat)
    return TOOL_HOSTS.antigravity.every(h => hasLocalHostEntry(entries, h));
  } catch {
    return false;
  }
}
...
function checkAllDNSStatus() {
  try {
    ...
    for (const [tool, hosts] of Object.entries(TOOL_HOSTS)) {
      result[tool] = hosts.every(h => hasLocalHostEntry(entries, h));
    }
    return result;
  } catch {
