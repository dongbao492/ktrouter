...
const LONG_QUOTA_RETRY_MS = 60 * 1000;
...
export class AntigravityExecutor extends BaseExecutor {
  ...
  parseQuotaResetFromBody(bodyText) {
    ...
    try {
      ...
      for (const detail of details) {
        ...
        const retryMs = this.parseRetryDuration(String(retryDelay || ""));
  ...
  parseError(response, bodyText) {
    ...
    try {
      const json = JSON.parse(bodyText);
      message = json?.error?.message || json?.message || bodyText;
    } catch { /* keep raw body */ }
    return {
      status: response.status,
      message,
      resetsAtMs: this.parseQuotaResetFromBody(bodyText)
    };
  }
  ...
  parseRetryDuration(value) {
    if (!value || typeof value !== "string") return null;
    const secondsOnly = value.match(/^(\d+(?:\.\d+)?)s$/);
    if (secondsOnly) return Math.ceil(Number(secondsOnly[1]) * 1000);

    const match = value.match(/(?:(\d+(?:\.\d+)?)h)?(?:(\d+(?:\.\d+)?)m)?(?:(\d+(?:\.\d+)?)s)?/i);
    if (!match || !match[0]) return null;

    let totalMs = 0;
    if (match[1]) totalMs += Number(match[1]) * 3600 * 1000;
    if (match[2]) totalMs += Number(match[2]) * 60 * 1000;
    if (match[3]) totalMs += Number(match[3]) * 1000;
        ...
        if (response.status === HTTP_STATUS.RATE_LIMITED || response.status === HTTP_STATUS.SERVICE_UNAVAILABLE) {
          ...
          if (!retryMs) {
            try {
              ...
              const quotaResetMs = this.parseQuotaResetFromBody(errorBody);
              if (quotaResetMs && quotaResetMs - Date.now() > LONG_QUOTA_RETRY_MS) {
                retryMs = quotaResetMs - Date.now();
                log?.debug?.("RETRY", `${response.status}, quota reset is too far away, trying fallback`);
