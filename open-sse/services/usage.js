...
const ANTIGRAVITY_QUOTA_MODELS = [
  {
    id: "gemini-3.5-flash-high",
    displayName: "Gemini 3.5 Flash (High)",
    aliases: ["gemini-3.5-flash-high", "gemini-3.5-flash", "gemini-3-flash-agent", "gemini-default"],
  },
  {
    id: "gemini-3.5-flash-medium",
    displayName: "Gemini 3.5 Flash (Medium)",
    aliases: ["gemini-3.5-flash-medium", "gemini-3-flash", "gemini-3.0-flash", "gemini-3.5-flash-low"],
  },
  {
    id: "gemini-3.1-pro-high",
    displayName: "Gemini 3.1 Pro (High)",
    aliases: ["gemini-3.1-pro-high", "gemini-3.1-pro", "gemini-pro-agent", "gemini-3-pro"],
  },
  {
  ...
  },
  {
    ...
    aliases: ["claude-sonnet-4-6-thinking", "claude-sonnet-4-6"],
  },
  {
    id: "claude-opus-4-6-thinking",
    displayName: "Claude Opus 4.6 (Thinking)",
    aliases: ["claude-opus-4-6-thinking", "claude-opus-4-6"],
  },
  {
    id: "gpt-oss-120b-medium",
    displayName: "GPT-OSS 120B (Medium)",
    aliases: ["gpt-oss-120b-medium", "gpt-oss-120b"],
  },
];

function pickAntigravityQuotaInfo(models, aliases) {
  const candidates = aliases
    .map((modelKey) => ({ modelKey, info: models[modelKey] }))
    .filter(({ info }) => info?.quotaInfo && !info.isInternal);

  if (candidates.length === 0) return null;

  candidates.sort((a, b) => {
    const remainingA = getAntigravityRemainingFraction(a.info.quotaInfo) ?? 1;
    const remainingB = getAntigravityRemainingFraction(b.info.quotaInfo) ?? 1;
    if (remainingA !== remainingB) return remainingA - remainingB;
    return aliases.indexOf(a.modelKey) - aliases.indexOf(b.modelKey);
  });

  return candidates[0];
}

function getAntigravityRemainingFraction(quotaInfo) {
  if (!quotaInfo) return null;
  if (quotaInfo.remainingFraction != null) {
    const n = Number(quotaInfo.remainingFraction);
    return Number.isFinite(n) ? Math.max(0, Math.min(1, n)) : null;
  }
  // Antigravity 2.0 daily endpoint omits remainingFraction for exhausted buckets
  // and only returns the resetTime. Treat that as 0 remaining.
  if (quotaInfo.resetTime) return 0;
  return null;
}
...
async function getAntigravityUsage(accessToken, providerSpecificData, proxyOptions = null) {
  try {
    ...
    for (const quotaApiUrl of ANTIGRAVITY_CONFIG.quotaApiUrls) {
      let response;
      try {
        response = await proxyAwareFetch(quotaApiUrl, {
          method: "POST",
          headers: {
            "Authorization": `Bearer ${accessToken}`,
      ...
      data.__quotaApiUrl = quotaApiUrl;
      break;
    }
    ...
    if (!data) {
      ...
      throw new Error(`Antigravity API error: ${lastResponse?.status || "unknown"}`);
    }

    const quotas = {};

    // Parse model quotas into canonical Antigravity UI model families.
    // Google can return different alias keys per account for the same quota bucket,
    // so keep a stable order and dedupe aliases here.
    if (data.models) {
      for (const model of ANTIGRAVITY_QUOTA_MODELS) {
        const selected = pickAntigravityQuotaInfo(data.models, model.aliases);
        if (!selected) continue;

        const { modelKey, info } = selected;
        const remainingFraction = getAntigravityRemainingFraction(info.quotaInfo);
        if (remainingFraction == null) continue;
        const remainingPercentage = remainingFraction * 100;

        // Antigravity exposes remaining quota. Keep used as remaining so the UI
        // reads 0/1000 as exhausted and 1000/1000 as fully available.
        ...
        quotas[model.id] = {
          ...
          sourceEndpoint: data.__quotaApiUrl,
        };
      }
    }
