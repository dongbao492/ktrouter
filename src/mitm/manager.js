...
const LINUX_ENV_DIR = path.join(os.homedir(), ".config", "environment.d");
const LINUX_ENV_FILE = path.join(LINUX_ENV_DIR, "ktrouter-mitm.conf");
const DESKTOP_MARKER = "KTRouter MITM NODE_EXTRA_CA_CERTS";

let mitmRestartCount = 0;
let mitmLastStartTime = 0;
let mitmIsRestarting = false;

function ensureWindowsNodeExtraCa(rootCACertPath) {
  if (!IS_WIN || !rootCACertPath || !fs.existsSync(rootCACertPath)) return;

  const normalized = path.resolve(rootCACertPath);
  process.env[NODE_EXTRA_CA_CERTS_ENV] = normalized;

  try {
    const existing = execSync(
      `reg query HKCU\\Environment /v ${NODE_EXTRA_CA_CERTS_ENV}`,
      { encoding: "utf8", windowsHide: true, stdio: ["ignore", "pipe", "ignore"] }
    );
    if (existing && existing.includes(normalized)) return;
  } catch { /* missing or unreadable env value, set it below */ }

  try {
    execSync(`setx ${NODE_EXTRA_CA_CERTS_ENV} "${normalized}"`, {
      windowsHide: true,
      stdio: "ignore",
    });
    log(`Cert: set ${NODE_EXTRA_CA_CERTS_ENV} for Windows user env`);
  } catch (e) {
    log(`Cert: could not set ${NODE_EXTRA_CA_CERTS_ENV}: ${e.message}`);
  }
}

function clearWindowsNodeExtraCa(rootCACertPath) {
  if (!IS_WIN || !rootCACertPath) return;

  const normalized = path.resolve(rootCACertPath).toLowerCase();
  const current = String(process.env[NODE_EXTRA_CA_CERTS_ENV] || "").toLowerCase();
  if (current === normalized) delete process.env[NODE_EXTRA_CA_CERTS_ENV];
  ...
  try {
    ...
    if (!existing.toLowerCase().includes(normalized)) return;
  } catch {
    return;
  }

  try {
    execSync(`reg delete HKCU\\Environment /v ${NODE_EXTRA_CA_CERTS_ENV} /f`, {
      windowsHide: true,
      stdio: "ignore",
    });
    log(`Cert: removed ${NODE_EXTRA_CA_CERTS_ENV} from Windows user env`);
  } catch (e) {
    log(`Cert: could not remove ${NODE_EXTRA_CA_CERTS_ENV}: ${e.message}`);
  }
}
...
function updateLinuxSessionEnv(rootCACertPath) {
  if (!IS_LINUX || !rootCACertPath) return;
  const normalized = path.resolve(rootCACertPath);
  process.env[NODE_EXTRA_CA_CERTS_ENV] = normalized;

  try {
    fs.mkdirSync(LINUX_ENV_DIR, { recursive: true });
    fs.writeFileSync(
      LINUX_ENV_FILE,
      `# ${DESKTOP_MARKER}\n${NODE_EXTRA_CA_CERTS_ENV}=${normalized}\n`,
      "utf8"
    );
  } catch (e) {
    log(`Cert: could not write Linux environment.d: ${e.message}`);
  }

  try {
    if (commandExists("systemctl")) {
      execSync(
        `systemctl --user set-environment ${NODE_EXTRA_CA_CERTS_ENV}=${shellQuoteSingle(normalized)}`,
  ...
  try {
    if (commandExists("dbus-update-activation-environment")) {
      execSync(
        `dbus-update-activation-environment --systemd ${NODE_EXTRA_CA_CERTS_ENV}`,
        {
          stdio: "ignore",
          windowsHide: true,
          env: { ...process.env, [NODE_EXTRA_CA_CERTS_ENV]: normalized },
        }
      );
    }
  } catch { /* best effort */ }
...
function clearLinuxSessionEnv(rootCACertPath) {
  if (!IS_LINUX) return;
  const normalized = rootCACertPath ? path.resolve(rootCACertPath) : "";
  if (!normalized || process.env[NODE_EXTRA_CA_CERTS_ENV] === normalized) {
    delete process.env[NODE_EXTRA_CA_CERTS_ENV];
  }

  try {
    if (fs.existsSync(LINUX_ENV_FILE)) {
      const content = fs.readFileSync(LINUX_ENV_FILE, "utf8");
      if (!normalized || content.includes(normalized) || content.includes(DESKTOP_MARKER)) {
        fs.unlinkSync(LINUX_ENV_FILE);
      }
    }
  } catch (e) {
    log(`Cert: could not remove Linux environment.d: ${e.message}`);
  }

  try {
    if (commandExists("systemctl")) {
      execSync(`systemctl --user unset-environment ${NODE_EXTRA_CA_CERTS_ENV}`, {
        stdio: "ignore",
        windowsHide: true,
      });
    }
...
function listLinuxDesktopCandidates() {
  if (!IS_LINUX) return [];
  const dirs = [
  ...
  for (const dir of dirs) {
    try {
      ...
      for (const name of fs.readdirSync(dir)) {
        ...
        if (/antigravity/i.test(name) || /antigravity/i.test(content)) found.push(file);
      }
...
function patchDesktopExecLine(line, rootCACertPath) {
  const execCmd = line.slice("Exec=".length).trim();
  if (!execCmd || execCmd.includes(DESKTOP_MARKER)) return line;

  const envValue = `${NODE_EXTRA_CA_CERTS_ENV}=${desktopQuote(rootCACertPath)}`;
  const cleaned = execCmd.replace(
    new RegExp(`(^|\\s)${NODE_EXTRA_CA_CERTS_ENV}=(?:"[^"]*"|'[^']*'|\\S+)`, "g"),
    "$1"
  ).replace(/\s+/g, " ").trim();

  if (/^(?:\/usr\/bin\/)?env(?:\s|$)/.test(cleaned)) {
    return `Exec=${cleaned.replace(/^(?:\/usr\/bin\/)?env/, (m) => `${m} ${envValue}`)}`;
  }
  return `Exec=env ${envValue} ${cleaned}`;
}

function patchAntigravityDesktopEntries(rootCACertPath) {
  if (!IS_LINUX || !rootCACertPath || !fs.existsSync(rootCACertPath)) return;
  const userAppsDir = path.join(os.homedir(), ".local", "share", "applications");
  try { fs.mkdirSync(userAppsDir, { recursive: true }); } catch { /* ignore */ }

  let patched = 0;
  for (const source of listLinuxDesktopCandidates()) {
    try {
      const target = source.startsWith(userAppsDir + path.sep)
        ? source
        : path.join(userAppsDir, path.basename(source));
      ...
      const next = current.split(/\r?\n/).map((line) => {
        ...
        return patchDesktopExecLine(line, path.resolve(rootCACertPath));
      }).join("\n");

      if (next !== current) {
        const backup = `${target}.ktrouter-bak`;
        if (!fs.existsSync(backup)) fs.writeFileSync(backup, current, "utf8");
        fs.writeFileSync(target, next, "utf8");
        patched++;
      }
    } catch (e) {
      log(`Cert: could not patch Antigravity desktop entry: ${e.message}`);
    }
  }

  if (patched > 0) {
    log(`Cert: patched ${patched} Antigravity desktop launcher(s) for ${NODE_EXTRA_CA_CERTS_ENV}`);
  }
}

function restoreAntigravityDesktopEntries(rootCACertPath) {
  if (!IS_LINUX) return;
  const normalized = rootCACertPath ? path.resolve(rootCACertPath) : "";
  for (const file of listLinuxDesktopCandidates()) {
    try {
      const backup = `${file}.ktrouter-bak`;
      if (!fs.existsSync(backup)) continue;
      const current = fs.readFileSync(file, "utf8");
      if (!current.includes(NODE_EXTRA_CA_CERTS_ENV)) continue;
      if (normalized && !current.includes(normalized)) continue;
      fs.copyFileSync(backup, file);
      fs.unlinkSync(backup);
      log(`Cert: restored Antigravity desktop launcher ${path.basename(file)}`);
    } catch (e) {
      log(`Cert: could not restore Antigravity desktop entry: ${e.message}`);
    }
  }
}

function ensureLinuxNodeExtraCa(rootCACertPath) {
  if (!IS_LINUX || !rootCACertPath || !fs.existsSync(rootCACertPath)) return;
  updateLinuxSessionEnv(rootCACertPath);
  patchAntigravityDesktopEntries(rootCACertPath);
  log(`Cert: set ${NODE_EXTRA_CA_CERTS_ENV} for Linux user env`);
}

function clearLinuxNodeExtraCa(rootCACertPath) {
  clearLinuxSessionEnv(rootCACertPath);
  restoreAntigravityDesktopEntries(rootCACertPath);
}

function ensureNodeExtraCa(rootCACertPath) {
  ensureWindowsNodeExtraCa(rootCACertPath);
  ensureLinuxNodeExtraCa(rootCACertPath);
}

function clearNodeExtraCa(rootCACertPath) {
  clearWindowsNodeExtraCa(rootCACertPath);
  clearLinuxNodeExtraCa(rootCACertPath);
}
...
async function syncSavedDnsToolState(actualStatus) {
  if (!_getSettings || !_updateSettings || !actualStatus) return;
  try {
    const settings = await _getSettings();
    const saved = settings.dnsToolEnabled || {};
    const next = { ...saved };
    let changed = false;

    for (const tool of Object.keys(TOOL_HOSTS)) {
      const actual = actualStatus[tool] === true;
      if (next[tool] !== actual) {
        next[tool] = actual;
        changed = true;
      }
    ...
    if (changed) await _updateSettings({ dnsToolEnabled: next });
  } catch (e) {
    err(`Failed to sync DNS state: ${e.message}`);
  }
}
...
async function getMitmStatus() {
  ...
  await syncSavedDnsToolState(dnsStatus);
...
async function scheduleMitmRestart(apiKey) {
  ...
  try {
    ...
    if (String(e.message || "").includes("MITM server is already running")) {
      log("MITM already running, restart not needed");
      mitmRestartCount = 0;
      mitmIsRestarting = false;
      return;
    }
...
async function startServer(apiKey, sudoPassword, forceKillPort443 = false) {
    ...
    try {
      if (fs.existsSync(PID_FILE)) {
        ...
        if (savedPid && isProcessAlive(savedPid)) {
          ...
          ensureNodeExtraCa(path.join(MITM_DIR, "rootCA.crt"));
  ...
  ensureNodeExtraCa(rootCACertPath);
  ...
  if (IS_WIN) {
    ...
    serverProcess = spawn(
      ...
      [effectiveServerPath],
      {
        ...
        env: {
          ...process.env,
          DATA_DIR: path.resolve(process.env.DATA_DIR || DATA_DIR),
...
async function stopServer(sudoPassword) {
  ...
  try { fs.unlinkSync(PID_FILE); } catch { /* ignore */ }
  clearNodeExtraCa(path.join(MITM_DIR, "rootCA.crt"));
