...
console.log("📦 Building QTRoute CLI package with Next.js...\n");
...
// Step 0b: Temporarily inject output: "standalone" into next.config.mjs
console.log("0️⃣ b Injecting output: standalone for CLI build...");
const nextConfigPath = path.join(appDir, "next.config.mjs");
const originalNextConfig = fs.readFileSync(nextConfigPath, "utf8");
if (!originalNextConfig.includes('output: "standalone"')) {
  const patched = originalNextConfig.replace(
    'const nextConfig = {',
    'const nextConfig = {\n  output: "standalone",'
  );
  fs.writeFileSync(nextConfigPath, patched, "utf8");
  console.log("✅ Injected standalone output\n");
} else {
  console.log("✅ Already has standalone output\n");
}
...
// Restore original next.config.mjs (remove standalone)
...
// Strip better-sqlite3 (native) — it lives in ~/.ktrouter/runtime to avoid
...
if (fs.existsSync(betterDir)) {
  fs.rmSync(betterDir, { recursive: true, force: true });
    console.log("✅ Stripped better-sqlite3 (lives in ~/.ktrouter/runtime)");
}
console.log("");
...
// Also copy to .next/static (server.js reads from .next/)
const staticDestAlt = path.join(cliAppDir, ".next", "static");
if (!fs.existsSync(staticDestAlt)) {
  const staticSrcFinal = fs.existsSync(staticSrcResolved) ? staticSrcResolved : staticSrc;
  copyRecursive(staticSrcFinal, staticDestAlt);
  console.log("✅ Also copied static to .next/static\n");
}
} else {
