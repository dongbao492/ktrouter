...
async function buildCodexHeader() {
  ...
  const { installed, hasKTRouter, config } = result.data;
...
async function buildDroidHeader() {
  ...
  const { installed, hasKTRouter, settings } = result.data;
  ...
  // Extract KTRouter custom model config
  const custom = settings?.customModels?.find(m => m.id === "custom:KTRouter-0");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenClawHeader() {
  ...
  // Extract KTRouter provider config
  const provider = settings?.models?.providers?.["ktrouter"];
  const primary = settings?.agents?.defaults?.model?.primary || "";
  const model = primary.startsWith("ktrouter/") ? primary.replace("ktrouter/", "") : (provider?.models?.[0]?.id || "");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenCodeHeader() {
  ...
  const { installed, hasKTRouter, opencode } = result.data;
...
async function showCliToolsMenu(port, breadcrumb = []) {
  ...
  await showMenuWithBack({
    title: "🔧 CLI Tools",
    breadcrumb,
    headerContent: `Configure CLI tools to use KTRouter\nEndpoint: ${endpoint}`,
    items: [
      {
        label: "Claude Code",
