...
const FILE_LOG_ENABLED = ["1", "true", "yes", "on"].includes(
  String(process.env.MITM_FILE_LOG || "").toLowerCase()
);
...
function redactObject(value) {
  ...
  for (const [key, item] of Object.entries(value)) {
    redacted[key] = isSensitiveKey(key) ? redactValue(item) : redactObject(item);
  }
  return redacted;
}
...
function dumpRequest(req, bodyBuffer, tag = "raw") {
  ...
  try {
    ...
    fs.writeFileSync(file, JSON.stringify({
      ...
      body: parsed !== null ? redactObject(parsed) : bodyBuffer.toString("utf8")
    }, null, 2));
    return file;
  } catch { return null; }
}
  ...
  return {
    ...
    writeChunk: (chunk) => {
      ...
      try {
        ...
        delete cleanHeaders["Content-Encoding"];
        const out = `STATUS: ${status}\nHEADERS: ${JSON.stringify(redactHeaders(cleanHeaders), null, 2)}\n---BODY---\n${text}`;
...
module.exports = { log, err, dumpRequest, createResponseDumper, isFileLoggingEnabled };
