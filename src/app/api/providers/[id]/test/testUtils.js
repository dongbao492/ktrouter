...
import { KILOCODE_CONFIG } from "@/lib/oauth/constants/oauth";
...
const OAUTH_TEST_CONFIG = {
  ...
  xai: {
    ...
    authHeader: "Authorization",
    authPrefix: "Bearer ",
    // xAI may return 403 for a valid token/account without API credit.
    // Treat it as auth accepted; chat usage will still surface real quota/credit errors.
    acceptStatuses: [403],
    refreshable: true,
  },
  gitlab: {
  ...
  codebuddy: { tokenExists: true, refreshable: true },
};
...
async function refreshOAuthToken(connection) {
  ...
  try {
    const tokens = await refreshTokenByProvider(connection.provider, {
...
async function testOAuthConnection(connection, effectiveProxy = null) {
  ...
  // Cursor/CodeBuddy can only verify that a token is present; do not fail them
  // just because there is no reliable probe or refresh endpoint.
  ...
  try {
    ...
    if ((res.status === 401 || res.status === 403) && config.refreshable && !refreshed && connection.refreshToken) {
      const tokens = await refreshOAuthToken(connection);
      if (tokens) {
...
export async function testSingleConnection(id) {
  ...
  if (result.refreshed && result.newTokens) {
    ...
    if (result.newTokens.copilotToken) {
      providerSpecificPatch.copilotToken = result.newTokens.copilotToken;
    }
    if (result.newTokens.copilotTokenExpiresAt) {
      providerSpecificPatch.copilotTokenExpiresAt = result.newTokens.copilotTokenExpiresAt;
    }
