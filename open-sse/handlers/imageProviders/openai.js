...
export default function createOpenAIAdapter(providerId) {
  return {
    ...
    buildBody: (model, body) => {
      const { prompt, n = 1, size = "1024x1024", quality, style, response_format, aspect_ratio, resolution } = body;
      // xAI only accepts a small image-generation parameter set.
      if (providerId === "xai") {
        const req = { model, prompt, n };
        if (response_format) req.response_format = response_format;
        if (aspect_ratio) req.aspect_ratio = aspect_ratio;
