...
import { KiroService } from "./services/kiro";
...
async function withNullTimeout(promise, timeoutMs) {
...
const PROVIDERS = {
  ...
  kiro: {
      ...
      if (data.accessToken) {
        return {
          ...
          data: {
            ...
            login_hint: data?.loginHint || data?.login_hint || null,
    ...
    postExchange: async (tokens) => {
      ...
      const email = await withNullTimeout(
        kiroService.resolveEmail({
          accessToken: tokens.access_token,
          refreshToken: tokens.refresh_token,
          tokenData: {
            idToken: tokens.id_token,
            email: tokens.email,
            loginHint: tokens.login_hint,
    ...
    mapTokens: (tokens, extra) => {
      const email = extra?.email || extractEmailFromAccessToken(tokens.access_token);
      const mapped = {
        accessToken: tokens.access_token,
        refreshToken: tokens.refresh_token,
        idToken: tokens.id_token,
...
// Backfill email for existing Kiro OAuth connections created before the
// device-code flow resolved the account identity.
export async function backfillKiroEmails() {
  ...
  try {
    ...
    for (const conn of targets) {
      ...
      if (email) {
        await updateProviderConnection(conn.id, { email });
      }
    }
  } catch (err) {
    kiroBackfillDone = false;
    console.log("backfillKiroEmails failed:", err?.message || err);
  }
}
