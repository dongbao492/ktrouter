...
Requires `NINEROUTER_URL` (and `NINEROUTER_KEY` if auth enabled). See https://raw.githubusercontent.com/decolua/ktrouter/refs/heads/master/skills/ktrouter/SKILL.md for setup.
...
curl -X POST $NINEROUTER_URL/v1/search \
  ...
  -d '{"model":"tavily/search","query":"KTRouter open source","max_results":5}'
```
