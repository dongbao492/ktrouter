...
import { canExecute, reportSuccess, reportFailure } from "../services/resilience/circuitBreaker.js";
import { isCooling, setCooldown } from "../services/resilience/cooldown.js";
import { extractSessionKey, bindSession } from "../services/resilience/sessionAffinity.js";
import { compressMessages, formatRtkLog } from "../rtk/index.js";

function countTranslatedMessages(body) {
  ...
  if (Array.isArray(body?.request?.contents)) return body.request.contents.length;

  const conversationState = body?.conversationState;
  if (conversationState) {
    const historyCount = Array.isArray(conversationState.history) ? conversationState.history.length : 0;
    return historyCount + (conversationState.currentMessage ? 1 : 0);
  }
...
export async function handleChatCore({ body, modelInfo, credentials, log, onCredentialsRefreshed, onRequestSuccess, onDisconnect, clientRawRequest, connectionId, userAgent, apiKey, ccFilterNaming, rtkEnabled, cavemanEnabled, cavemanLevel, sourceFormatOverride, providerThinking }) {
  ...
  const msgCount = countTranslatedMessages(translatedBody);
  ...
  // Resilience: circuit breaker + cooldown check
  const cbCheck = canExecute(provider);
  if (!cbCheck.allow) {
    trackPendingRequest(model, provider, connectionId, false, true);
    log?.warn?.("CIRCUIT", `${provider.toUpperCase()} | ${cbCheck.reason}`);
    return createErrorResult(HTTP_STATUS.SERVICE_UNAVAILABLE, `Provider ${provider} temporarily unavailable (${cbCheck.reason})`);
  }
  const credKey = connectionId || credentials?.apiKey || "";
  const sessionKey = extractSessionKey(body, clientRawRequest?.headers || {});
  if (sessionKey) {
    log?.debug?.("SESSION", `${provider.toUpperCase()} | session=${sessionKey.slice(0, 24)}`);
  }
  if (isCooling(credKey)) {
    trackPendingRequest(model, provider, connectionId, false, true);
    log?.warn?.("COOLDOWN", `${provider.toUpperCase()} | credential cooling down`);
    return createErrorResult(HTTP_STATUS.RATE_LIMITED, `Credential for ${provider} is cooling down`);
  }
  ...
  if (!executor.noAuth && (providerResponse.status === HTTP_STATUS.UNAUTHORIZED || providerResponse.status === HTTP_STATUS.FORBIDDEN)) {
    try {
      const newCredentials = await refreshProviderCredentials(provider, credentials, log, {
        refreshFn: () => executor.refreshCredentials(credentials, log, proxyOptions),
      });
      if (isUnrecoverableRefreshError(newCredentials)) {
        const message = `${provider} refresh token is invalid or expired. Please re-authorize this provider.`;
        log?.warn?.("TOKEN", message);
        providerResponse = new Response(JSON.stringify({ error: { message } }), {
  ...
  if (providerResponse.ok) {
    ...
    if (sessionKey && connectionId) bindSession(sessionKey, connectionId);
  }

  // Provider returned error
  if (!providerResponse.ok) {
    reportFailure(provider, providerResponse.status);
    trackPendingRequest(model, provider, connectionId, false, true);
    const { statusCode, message, resetsAtMs } = await parseUpstreamError(providerResponse, executor);
    if ((providerResponse.status === 429 || providerResponse.status === 403) && provider !== "antigravity") {
      const cooldownMs = resetsAtMs && resetsAtMs > Date.now() ? resetsAtMs - Date.now() : undefined;
      setCooldown(credKey, providerResponse.status, cooldownMs);
    }
