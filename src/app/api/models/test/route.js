...
export async function POST(request) {
  try {
    ...
    // Forward dashboard auth cookie so internal fetch passes dashboardGuard.
    ...
    if (kind === "image") {
      ...
      const hasImage = !!(firstImage?.b64_json || firstImage?.url);
      if (!hasImage) {
    ...
    if (kind === "video") {
      ...
      const hasVideo = !!firstVideo?.url;
      if (!hasVideo) {
        return NextResponse.json({ ok: false, latencyMs, status: res.status, error: "Provider returned no video data" });
      }
      return NextResponse.json({ ok: true, latencyMs, error: null, status: res.status });
    }
