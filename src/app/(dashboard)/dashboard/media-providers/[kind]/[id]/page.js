...
// Shared row layout â€” defined outside components to avoid re-mount on re-render
...
const KIND_EXAMPLE_CONFIG = {
  ...
  image: {
    ...
    extraFields: [
      ...
      { key: "aspect_ratio", label: "Aspect", type: "select", default: "", options: ["", "1:1", "16:9", "9:16", "4:3", "3:4"] },
      { key: "resolution", label: "Resolution", type: "select", default: "", options: ["", "low", "medium", "high"] },
...
function EmbeddingExampleCard({ providerId, customAlias }) {
  ...
  // Build request body â€” include dimensions only if user provided a positive number
  const buildBody = () => {
  ...
  return (
    <Card>
      ...
      <div className="flex flex-col gap-2.5">
        {/* Model â€” text input for custom node, dropdown otherwise */}
        <Row label="Model">
          {isCustom ? (
            <input
              value={selectedModel}
        ...
        {/* Dimensions (optional) â€” truncate embedding vector length */}
        ...
        {/* Response â€” default example or real result */}
        <div>
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between mb-1.5">
...
// â”€â”€â”€ TTS Example Card â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function TtsExampleCard({ providerId }) {
  ...
  // Open modal â€” load language list
  const openModal = async () => {
    ...
    try {
      if (config.voiceSource === "hardcoded") {
        ...
        const providerName = providerId === "local-device" ? "local-device" : "edge-tts";
        let url = "/api/media-providers/tts/voices?provider=" + providerName;
        if (config.apiEndpoint) url = config.apiEndpoint;
  ...
  // Click language â†’ close modal â†’ show voices below
    ...
    <>
      <Card>
        ...
        <div className="flex flex-col gap-2.5">
          ...
          {/* Model selector â€” prefer ttsConfig.models, else providerModels via modelKey */}
          ...
          {/* Language hint dropdown (Gemini) â€” sends body.language to guide pronunciation */}
          ...
          {/* Voice chips â€” shown after language picked (edge-tts, local-device) or always (OpenAI/ElevenLabs) */}
            ...
            <Row label="Voice">
              <div className="flex flex-wrap gap-1.5">
                {countryVoices.map((v) => (
                  ...
                  >
                    {v.name}{v.gender ? ` Â· ${v.gender[0].toUpperCase()}` : ""}
          ...
          {/* Voice ID input (ElevenLabs) â€” manual entry or auto-fill from chip */}
...
// Generic Example Card â€” config-driven for webSearch, webFetch, image, imageToText, stt, video, music
function GenericExampleCard({ providerId, kind }) {
  ...
  const handleImageUpload = (e) => {
    ...
    reader.onload = (event) => {
      if (event.target?.result) {
        setRefImage(event.target.result);
      }
    };
  ...
  const truncateBase64 = (val) => {
    if (typeof val === "string" && val.startsWith("data:") && val.includes("base64,")) {
      const parts = val.split("base64,");
      return `${parts[0]}base64,... [${val.length} chars]`;
    }
    return val;
  };

  const getCurlBody = (body) => {
    const clone = JSON.parse(JSON.stringify(body));
    if (clone.image) clone.image = truncateBase64(clone.image);
    if (clone.mask_image) clone.mask_image = truncateBase64(clone.mask_image);
    return clone;
  };
  ...
  const requestBody = {
    ...
    ...exConfig.extraBody,
    ...extraBodyFromFields,
    ...((supportsEdit || kind === "video") && effectiveRefImage ? { image: effectiveRefImage } : {}),
    ...(supportsMask && effectiveMaskImage ? { mask_image: effectiveMaskImage } : {}),
  };

  // Streaming supported for codex image (Plus/Pro accounts) â€” disabled when binary output requested
  ...
  const curlBody = getCurlBody(requestBody);
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      // Binary image response â€” convert to blob URL
  ...
  return (
    <Card>
      <h2 className="text-lg font-semibold mb-4">Example</h2>
      <div className="flex flex-col gap-2.5">
        {/* Model selector â€” dropdown if presets exist, else manual input for media kinds */}
        {kindModels.length > 0 ? (
          <Row label="Model">
            <select
              value={selectedModel}
        ...
        {/* Reference image (only for edit-capable image models or video models) */}
        {(supportsEdit || kind === "video") && (
          <Row label="Ref Image (URL)">
            <div className="flex flex-col gap-2">
              <div className="relative">
                <input
                  value={refImage.startsWith("data:") ? `${refImage.split(";")[0]};base64,... [${refImage.length} characters]` : refImage}
                  onChange={(e) => {
                    if (!refImage.startsWith("data:")) {
                      setRefImage(e.target.value);
                    }
                  }}
                  readOnly={refImage.startsWith("data:")}
                  placeholder={imageEditDefaults.image || (kind === "video" ? "https://pub-1fb693cb11cc46b2b2f656f51e015a2c.r2.dev/dog.png" : "https://example.com/source.png")}
                  className="w-full px-3 py-1.5 pr-14 text-sm border border-border rounded-lg bg-background focus:outline-none focus:border-primary text-ellipsis overflow-hidden"
                />
                <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-1.5">
                  ...
                  <label className="cursor-pointer text-text-muted hover:text-primary transition-colors flex items-center">
        ...
        {supportsMask && (
          <Row label="Mask (URL)">
            <div className="flex flex-col gap-2">
              <div className="relative">
                <input
                  value={maskImage.startsWith("data:") ? `${maskImage.split(";")[0]};base64,... [${maskImage.length} characters]` : maskImage}
                  onChange={(e) => {
                    if (!maskImage.startsWith("data:")) {
                      setMaskImage(e.target.value);
                    }
                  }}
                  readOnly={maskImage.startsWith("data:")}
        ...
        {/* Extra fields â€” for kinds without model concept (webSearch/webFetch), show all; otherwise filter by model.params */}
        ...
        {/* Output Format toggle (image only) â€” last */}
        {kind === "image" && (
          <Row label="Output Format">
            <select
        ...
        {(running || progress) && useStreaming && (
          <div className="flex flex-col gap-2 px-3 py-2 rounded-lg bg-sidebar border border-border sm:flex-row sm:items-center sm:gap-3">
            ...
            <span className="text-xs text-text-muted">
              ...
              {!running && progress?.bytesReceived ? ` Â· ${(progress.bytesReceived / 1024).toFixed(1)} KB` : ""}
            </span>
          </div>
        )}
          ...
          {kind === "video" && result?.data?.data?.[0]?.url && (
            <div className="mt-2">
              <div className="flex items-center justify-end gap-3 mb-1.5">
                <a
                  href={`/api/media/download?url=${encodeURIComponent(result.data.data[0].url)}`}
                  className="inline-flex items-center gap-1 text-xs text-text-muted hover:text-primary transition-colors cursor-pointer"
                >
                  <span className="material-symbols-outlined text-[14px]">download</span>
                  Download
                </a>
...
// â”€â”€â”€ STT Example Card â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
          ...
          <div className="flex flex-col gap-2">
            ...
            {audioFile && (
              <span className="text-xs text-text-muted font-mono">
                {audioFile.name} Â· {(audioFile.size / 1024).toFixed(1)} KB
              </span>
            )}
          </div>
        </Row>
...
export default function MediaProviderDetailPage() {
  ...
  return (
    <div className="flex flex-col gap-8">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-4">
          ...
          <div className="flex-1">
            ...
            <div className="flex items-center gap-1.5 mt-1 flex-wrap">
              {isCustom && <Badge variant="default" size="sm">Custom Â· {customNode?.prefix}</Badge>}
              {kinds.map((k) => (
      ...
      {/* Provider Info â€” config-driven, supports searchConfig, fetchConfig, ttsConfig, embeddingConfig, searchViaChat */}
