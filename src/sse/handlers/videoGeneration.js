...
// Providers that don't require credentials (if any)
...
export async function handleVideoGeneration(request) {
  ...
  if (!body.prompt && !body.image) return errorResponse(HTTP_STATUS.BAD_REQUEST, "Missing required field: prompt or image");
  ...
  if (comboModels) {
    ...
    return handleComboChat({
      body,
      models: comboModels,
      handleSingleModel: (b, m) => handleSingleModelVideo(b, m, { wantsStream, binaryOutput, preferredConnectionId }),
      log,
      comboName: modelStr,
      comboStrategy,
      comboStickyLimit,
  ...
  return handleSingleModelVideo(body, modelStr, { wantsStream, binaryOutput, preferredConnectionId });
}

async function handleSingleModelVideo(body, modelStr, { wantsStream, binaryOutput, preferredConnectionId } = {}) {
