...
// Self-heal SQLite runtime deps (sql.js + better-sqlite3) into ~/.ktrouter/runtime
...
// Identifiers for killAllAppProcesses - only kill ktrouter specifically
const PROCESS_IDENTIFIERS = [
  'ktrouter'  // Only package name - avoid killing other apps
];
    ...
    try {
      ...
      if (platform === "win32") {
        ...
        try {
          ...
          const lines = output.split("\n").slice(1).filter(l => l.trim());
          lines.forEach(line => {
            // Whitelist: real node process running ktrouter/cli.js, or next-server.
            // Avoids killing editors/grep/strace/cursor that just have "ktrouter" in cmdline.
            const cmd = line.toLowerCase();
            const isAppProcess =
              (cmd.includes("node") && cmd.includes("ktrouter") && (cmd.includes("cli.js") || cmd.includes("\\ktrouter") || cmd.includes("/ktrouter")))
        ...
        try {
          ...
          lines.forEach(line => {
            // Whitelist: real node process running ktrouter/cli.js, or next-server.
            // Avoids killing grep/strace/editors/cursor that incidentally match "ktrouter".
            const cmd = line.toLowerCase();
            const isAppProcess =
              (cmd.includes("node") && cmd.includes("ktrouter") && (cmd.includes("cli.js") || cmd.includes("/ktrouter")))
              || cmd.includes("next-server");
            if (isAppProcess) {
              const parts = line.trim().split(/\s+/);
              const pid = parts[1];
...
// Load .env file from standalone app dir
...
function startServer(latestVersion) {
      ...
      while (true) {
        ...
        if (choice === "update") {
          ...
          if (process.platform === "darwin") {
            ...
            console.log(`🔔 KTRouter is running in tray (PID: ${process.pid})`);
            console.log(`   Server: http://${displayHost}:${port}`);
          ...
          console.log(`🔔 KTRouter is now running in background (PID: ${bgProcess.pid})`);
          console.log(`   Server: http://${displayHost}:${port}`);
          console.log(`\n💡 You can close this terminal. Right-click tray icon to quit.\n`);
  ...
  function tryRestart(code) {
    ...
    if (restartCount >= MAX_RESTARTS) {
      ...
      try {
        const dbPath = path.join(os.homedir(), process.platform === "win32" ? path.join("AppData", "Roaming", "ktrouter", "db.json") : path.join(".ktrouter", "db.json"));
