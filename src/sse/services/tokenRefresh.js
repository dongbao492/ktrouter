...
import {
  ...
  refreshKimiCodingToken as _refreshKimiCodingToken,
  refreshCodeBuddyToken as _refreshCodeBuddyToken,
  refreshGitLabToken as _refreshGitLabToken,
...
export const refreshClineToken = (refreshToken) =>
  _refreshClineToken(refreshToken, log);

export const refreshKimiCodingToken = (refreshToken) =>
  _refreshKimiCodingToken(refreshToken, log);

export const refreshCodeBuddyToken = (refreshToken) =>
  _refreshCodeBuddyToken(refreshToken, log);

export const refreshGitLabToken = (refreshToken, credentials) =>
  _refreshGitLabToken(refreshToken, credentials, log);
...
export const refreshProviderCredentials = (provider, credentials, options = {}) =>
  _refreshProviderCredentials(provider, credentials, log, options);
...
export async function updateProviderCredentials(connectionId, newCredentials) {
  try {
    ...
    const providerSpecificPatch = {
      ...(newCredentials.providerSpecificData || {}),
    };
    if (newCredentials.copilotToken) {
      providerSpecificPatch.copilotToken = newCredentials.copilotToken;
    }
    if (newCredentials.copilotTokenExpiresAt) {
      providerSpecificPatch.copilotTokenExpiresAt = newCredentials.copilotTokenExpiresAt;
    }

    if (Object.keys(providerSpecificPatch).length > 0) {
      const existingConnection = newCredentials.existingProviderSpecificData
        ? null
    ...
    if (remaining < refreshLead) {
      ...
      if (newCreds?.accessToken) {
        ...
        creds = {
          ...
          copilotToken:
            newCreds.copilotToken ??
            newCreds.providerSpecificData?.copilotToken ??
            creds.copilotToken,
          copilotTokenExpiresAt:
            newCreds.copilotTokenExpiresAt ??
            newCreds.providerSpecificData?.copilotTokenExpiresAt ??
            creds.copilotTokenExpiresAt,
