...
function compressText(text, stats, shape) {
  ...
  const primaryFn = fn || smartTruncate;
  let out = safeApply(primaryFn, text);
  let usedFilter = primaryFn;

  // Fallback: if primary filter doesn't help, try smartTruncate for long plain/unstructured outputs.
  if ((!out || out.length === 0 || out.length >= bytesIn) && primaryFn !== smartTruncate) {
    const fallbackOut = safeApply(smartTruncate, text);
    if (fallbackOut && fallbackOut.length > 0 && fallbackOut.length < bytesIn) {
      out = fallbackOut;
      usedFilter = smartTruncate;
    }
  }
  ...
  stats.hits.push({ shape, filter: usedFilter.filterName || usedFilter.name, saved: bytesIn - out.length });
  return out;
}
