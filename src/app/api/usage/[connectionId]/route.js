...
import { refreshProviderCredentials, isUnrecoverableRefreshError } from "@/sse/services/tokenRefresh";
...
async function refreshAndUpdateCredentials(connection, force = false, proxyOptions = null) {
  ...
  // Use the shared refresh path so concurrent refreshes share one request.
  const refreshResult = await refreshProviderCredentials(connection.provider, credentials, {
    refreshFn: () => executor.refreshCredentials(credentials, console, proxyOptions),
  });
  ...
  if (isUnrecoverableRefreshError(refreshResult)) {
    throw new Error("Refresh token is invalid or expired. Please re-authorize the connection.");
  }
