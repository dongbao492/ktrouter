...
export default function MitmToolCard({
  ...
  const saveMappings = useCallback(async (mappings) => {
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(data.error || "Failed to save model mappings");
      }
      return true;
    } catch (error) {
      setWarning(error.message || "Failed to save model mappings");
      return false;
    }
  }, [tool.id]);
  ...
  const openModelSelector = (alias) => {
    if (!dnsActive || !hasActiveProviders) return;
    setCurrentEditingAlias(alias);
    setModalOpen(true);
  };

  const handleModelSelect = (model) => {
    if (!dnsActive || !currentEditingAlias || model.isPlaceholder) return;
          ...
          <div className="mt-4 pt-4 border-t border-border flex flex-col gap-4">
            ...
            {tool.defaultModels?.length > 0 && (
              <div className="flex flex-col gap-2">
                  ...
                  <div key={model.alias} className="grid grid-cols-1 gap-1.5 sm:grid-cols-[9rem_auto_1fr_auto] sm:items-center sm:gap-2">
                    ...
                    <div className="relative w-full min-w-0">
                      <input
                        ...
                        className="w-full min-w-0 pl-2 pr-7 py-2 bg-surface rounded border border-border text-xs focus:outline-none focus:ring-1 focus:ring-primary/50 disabled:cursor-not-allowed disabled:opacity-50 sm:py-1.5"
                      />
                      {dnsActive && modelMappings[model.alias] && (
                        <button
                          onClick={() => {
                    ...
                    <button
                      ...
                      disabled={!dnsActive || !hasActiveProviders}
                      className={`rounded border px-2 py-2 text-xs transition-colors sm:py-1.5 ${dnsActive && hasActiveProviders ? "bg-surface border-border hover:border-primary cursor-pointer" : "opacity-50 cursor-not-allowed border-border"}`}
                    >
                      Select
                    </button>
                  </div>
                ...
                {!dnsActive && (
                  <p className="px-1 text-[11px] text-text-muted">Start DNS before selecting or editing model mappings.</p>
                )}
              </div>
            )}
