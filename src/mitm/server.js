...
const { log, err, dumpRequest, createResponseDumper, isFileLoggingEnabled } = require("./logger");
const { TARGET_HOSTS, URL_PATTERNS, MODEL_SYNONYMS, MODEL_PATTERNS, getToolForHost } = require("./config");
const { DATA_DIR, MITM_DIR } = require("./paths");
const { getCertForDomain } = require("./cert/generate");
const { getMitmAlias } = require("./dbReader");
const LOCAL_PORT = 443;
const IS_WIN = process.platform === "win32";
const ENABLE_FILE_LOG = isFileLoggingEnabled();
const INTERNAL_REQUEST_HEADER = { name: "x-request-source", value: "local" };

// Keep passthrough traffic on the original host by default.
// Antigravity 2.0 uses non-chat endpoints for login/account/quota/model bootstrap;
// rewriting those to the daily endpoint can make the desktop app think the user is
// not logged in even when the production endpoint accepts the token.
const HOST_REWRITE = process.env.MITM_REWRITE_ANTIGRAVITY_PASSTHROUGH === "1"
...
function getMappedModel(tool, model) {
  ...
  try {
    ...
    // Fallback for new model names to maintain compatibility with existing aliases.json
    if (tool === "antigravity") {
      const firstAlias = (...keys) => {
        const key = keys.find(k => aliases[k]);
        return key ? aliases[key] : null;
      };
      let fallback = null;
      if (lookup === "gemini-3.5-flash-high") {
        fallback = firstAlias("gemini-3.5-flash-high", "gemini-3-flash-agent", "gemini-3.5-flash", "gemini-3-flash");
      } else if (lookup === "gemini-3.5-flash-medium") {
        fallback = firstAlias("gemini-3.5-flash-medium", "gemini-3-flash", "gemini-3.0-flash", "gemini-3.5-flash-low");
      } else if (lookup === "gemini-pro-agent" || lookup === "gemini-3.1-pro-high") {
        fallback = firstAlias("gemini-3.1-pro-high", "gemini-pro-agent", "gemini-3.1-pro");
      } else if (lookup === "gemini-3-flash-agent" || lookup === "gemini-3-flash") {
        fallback = firstAlias("gemini-3-flash", "gemini-3-flash-agent", "gemini-3.0-flash");
      } else if (lookup === "gemini-3.5-flash-low") {
        fallback = firstAlias("gemini-3.5-flash-low", "gemini-3.5-flash-medium", "gemini-3-flash");
      } else if (lookup === "claude-sonnet-4-6-thinking") {
        fallback = firstAlias("claude-sonnet-4-6-thinking", "claude-sonnet-4-6");
      } else if (lookup === "claude-opus-4-6-thinking") {
        fallback = firstAlias("claude-opus-4-6-thinking", "claude-opus-4-6");
      } else if (lookup === "gpt-oss-120b-medium") {
        fallback = firstAlias("gpt-oss-120b-medium", "gpt-oss-120b");
      }
      if (fallback) return fallback;
    }

    const pattern = MODEL_PATTERNS?.[tool]?.find(({ match }) => match.test(model) || match.test(lookup));
    if (pattern?.alias) {
      if (aliases[pattern.alias]) return aliases[pattern.alias];
      if (tool === "antigravity") {
        const legacyAlias = {
          "gemini-3.5-flash-high": ["gemini-3-flash-agent", "gemini-3.5-flash", "gemini-3-flash"],
          "gemini-3.5-flash-medium": ["gemini-3-flash", "gemini-3.0-flash", "gemini-3.5-flash-low"],
          "gemini-3.5-flash-low": ["gemini-3.5-flash-medium", "gemini-3-flash"],
          "claude-sonnet-4-6-thinking": ["claude-sonnet-4-6"],
          "claude-opus-4-6-thinking": ["claude-opus-4-6"],
          "gpt-oss-120b-medium": ["gpt-oss-120b"],
        }[pattern.alias]?.find(k => aliases[k]);
        if (legacyAlias) return aliases[legacyAlias];
      }
    }
...
const server = https.createServer(sslOptions, async (req, res) => {
  try {
    ...
    // Antigravity now frequently calls /v1internal:* endpoints without a direct
    // model field in URL/body. In that case we still intercept and let KTRouter
    // auto-detect/route the request instead of silently passthrough.
    if (tool === "antigravity") {
