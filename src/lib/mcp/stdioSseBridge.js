...
const getCustomStore = () => {
  if (!globalThis.__ktrouterCustomPlugins) globalThis.__ktrouterCustomPlugins = new Map();
  return globalThis.__ktrouterCustomPlugins;
};
...
function getOrSpawn(name) {
  ...
  // On Windows, spawn "npx" needs shell:true or use "npx.cmd"
  const cmd = process.platform === "win32" && plugin.command === "npx" ? "npx.cmd" : plugin.command;
  let proc;
  try {
    proc = spawn(cmd, plugin.args, { stdio: ["pipe", "pipe", "pipe"], env: process.env, windowsHide: true });
  } catch (e) {
    console.warn(`[mcp:${name}] spawn failed: ${e.message}`);
    throw new Error(`Failed to spawn MCP plugin "${name}": ${e.message}`);
  }
  proc.on("error", (e) => { console.warn(`[mcp:${name}] process error: ${e.message}`); store.delete(name); });
