...
function checkCertInstalledWindows(certPath) {
  return new Promise((resolve) => {
    // Check by SHA1 fingerprint in both LocalMachine and CurrentUser Root stores
    let fingerprint;
    try {
    ...
    exec(`certutil -store Root ${fingerprint}`, { windowsHide: true }, (error) => {
      if (!error) return resolve(true);
      // Fallback: check CurrentUser store (no admin required)
      exec(`certutil -user -store Root ${fingerprint}`, { windowsHide: true }, (error2) => {
        resolve(!error2);
      });
    });
  });
...
async function installCertMac(sudoPassword, certPath) {
  ...
  try {
    ...
    log("ÃƒÂ°Ã…Â¸Ã¢â‚¬ÂÃ‚Â Cert: ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ installed to system keychain");
  } catch (error) {
...
async function installCertWindows(certPath) {
  ...
  // Delete ALL stale certs with same CN before adding to avoid duplicates.
  const userFlag = isAdmin() ? "" : "-user ";
  const script = `
    for ($i = 0; $i -lt 10; $i++) {
      & certutil ${userFlag}-delstore Root ${quotePs(ROOT_CA_CN)} 2>$null | Out-Null
      if ($LASTEXITCODE -ne 0) { break }
    }
    $exit = & certutil ${userFlag}-addstore Root ${quotePs(certPath)} 2>&1
...
async function uninstallCertMac(sudoPassword, certPath) {
  ...
  try {
    ...
    log("ÃƒÂ°Ã…Â¸Ã¢â‚¬ÂÃ‚Â Cert: ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ uninstalled from system keychain");
  } catch (err) {
...
async function uninstallCertWindows() {
  ...
  try {
    await runElevatedPowerShell(script);
    log("ÃƒÂ°Ã…Â¸Ã¢â‚¬ÂÃ‚Â Cert: ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ uninstalled from Windows Root store");
  } catch (e) {
...
async function installCertLinux(sudoPassword, certPath) {
  if (!isSudoAvailable()) {
    log(`ÃƒÂ°Ã…Â¸Ã¢â‚¬ÂÃ‚Â Cert: cannot install to system store without sudo ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â trust this file on clients: ${certPath}`);
  ...
  try {
    ...
    await updateNssDatabases(certPath, 'add');
    log(`ÃƒÂ°Ã…Â¸Ã¢â‚¬ÂÃ‚Â Cert: ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ installed to Linux trust store (${config.dir}) and user browser databases`);
  } catch (error) {
...
async function uninstallCertLinux(sudoPassword) {
  ...
  try {
    await execWithPassword(cmd, sudoPassword);
    log("ÃƒÂ°Ã…Â¸Ã¢â‚¬ÂÃ‚Â Cert: ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ uninstalled from Linux trust store and user browser databases");
  } catch (error) {
    throw new Error("Failed to uninstall certificate");
  }
}
